...
Here, we are going to read in spatial data slightly differently to the shapefile tutorial to demonstrate an alternative way. This approach uses the `readOGR` function from the `rgdal` package, and a variable from the Active Lives survey to map.
...
#Basic Cartogram
Using the `cartogram` package, we can plot a map using the spatial dataframe we have imported and joined with Active Lives data.
