...
sg_df <- read.csv('SG_all.csv', header = TRUE, na.strings = "N/A")
...
These figures are for funded projects that have started during our current strategy period (from January 2016 - November 2019):

* Number of Applications: `r nrow(sg_df)`
* Total amount awarded through our Small Grants fund: `r paste0("£", formatC(sum(sg_df$AWARD), format="f", digits=2, big.mark=","))`
...
table_df <- sg_df %>%
  ...
  select(c(`Award Amount`, `Total Project Cost`, `Government Outcome`, "Sport", "Region", `Organisation Type`, "Focus"))
summary(table_df)
```
...
ggplot(sg_df)+
  ...
  labs( x= "Amount Awarded", y = "Total Project Cost", title = "Funding Amount by Focus")+
  scale_y_continuous(labels = scales::dollar_format(prefix = "£")) +
  scale_x_continuous(labels = scales::dollar_format(prefix = "£")) 
```

## Awards by Outcome
```{r go_col, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
ggplot(sg_df)+
...
```{r go, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
ggplot(sg_df)+
  geom_boxplot(aes(Govt_Outcome, AWARD), fill = "#5D4086" ) +
  theme(text = element_text(family = "Norwester", size = 12, color = "#525252"),
        plot.title=element_text(size=14,family = "Norwester", face="bold", hjust = 0.5, color = "#525252"),
        axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid = element_blank(),
        ...
        plot.caption=element_text(size=9, hjust=0, margin=margin(15,0,0,0)),
        legend.position = "right") +
  labs( x= "Government Outcome", y = "Amount Awarded", title = "Amount Awarded per Government Outcome")+
  scale_y_continuous(labels = scales::dollar_format(prefix = "£"))
```
...
sg_df %>%
  ...
  theme(text = element_text(family = "Norwester", size = 12, color = "#525252"),
        ...
        plot.caption=element_text(size=9, hjust=0, margin=margin(15,0,0,0)),
        legend.position = "right") +
  labs( x= "Organisation type", y = "Number of Applications", title = "Type of Sport by Number of Applications")
```
...
sg_df %>%
  ...
  theme(text = element_text(family = "Norwester", size = 12, color = "#525252"),
        ...
        plot.caption=element_text(size=9, hjust=0, margin=margin(15,0,0,0)),
        legend.position = "right") +
  labs( x= "Organisation type", y = "Number of Applications", title = "Number of Applications by Organisation Type")
```

## How many times funded?
This graph shows the distribution of funding more than once. As the histogram below shows, the maximum number of separate funded projects for a single organisation was `r max(sp_org$n)`

```{r fund, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center"}

ggplot(sp_org)+
  geom_histogram(aes(n), fill = "#5D4086" , binwidth = 1)+
  theme(text = element_text(family = "Norwester", size = 12, color = "#525252"),
        plot.title=element_text(size=14,family = "Norwester", face="bold", hjust = 0.5, color = "#525252"),
        axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid = element_blank(),
        ...
        plot.caption=element_text(size=9, hjust=0, margin=margin(15,0,0,0)),
        legend.position = "left")  +
  labs( x= "", y = "Frequency funded", title = "Number of Times an Organisation was Funded")
...
* The top LA for number of applications is: `r sp_la[1,1]` with `r sp_la[1,2]` applications
...
ggplot(sg_df)+
  ...
  labs( x= "Decile", y = "Number of applications", title = "IMD Decile distribution",
...
# Text Analysis 
Using the "12 month progress" field, we can look at the unstructured data of the Measure and Evaluation element of an application. We can do this using word frequency, and also topic extraction using LDA (Latent Dirichlet Modelling). 
...
## Sentiment
Number of Small Grants projects with comment in the "12 month progress" field: `r nrow(sentiment_df)`
