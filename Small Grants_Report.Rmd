...
## Gender
In general, the Small Grants fund funds projects that have more male participants than women. 

The below table shows the average target numbers and percentage for women:

```{r women_target,  echo=FALSE, message=FALSE, warning=FALSE, fig.align="center"}

me_df_women.num <- me_df_women[,c(3,4,6,8)]
kable(as.data.frame(colMeans(me_df_women.num)), col.names = "Figures Overall", digits = 0)%>%
  kable_styling(bootstrap_options = "bordered",
                full_width = FALSE) 
...
The below table shows the average target numbers and percentage for men:

```{r women_actuals,  echo=FALSE, message=FALSE, warning=FALSE, fig.align="center"}

me_df_women.num <- me_df_women[,c(1,2,5,7)]
...
The below chart shows how much is invested in projects tht have a specific focus (focus defined as more participants at 12 months of a specific gender):

```{r women_focus, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center"}

ggplot(me_box_g)+
  geom_col(aes(x= Gender_Focus, y = AWARD, fill = prediction) )+
  theme(text = element_text(family = "Norwester", size = 12, color = "#5F5F5F"),
        plot.title=element_text(size=14,family = "Norwester", face="bold", hjust = 0.5, color = "#525252"),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 7),
        panel.grid = element_blank(),
        ...
        plot.caption=element_text(size=9, hjust=0, margin=margin(15,0,0,0)),
        legend.position = "left")  +
  labs( x= "Gender focus", y = "Amount awarded", title = "Amount Awarded by Gender: Success of Project") +
  scale_y_continuous(labels = scales::dollar_format(prefix = "£")) +
  scale_fill_manual(values = se_colour, name = "Is the project \nsuccessful?")
